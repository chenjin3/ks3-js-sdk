...
! function(e, t) {
	...
	i(u, [], function() {
				...
				init: function() {
					a.convertEventPropsToHandlers(c), a.bind("RuntimeInit", function(e, t) {
						a.ruid = t.uid, a.bind("Drop", function() {
							...
							var path = t.exec.call(a, 'FileDrop', 'getFilesPath');
							a.files = [], i.each(e, function(e, index) {
								var temp = new r(a.ruid, e);
								temp.webkitRelativePath = path[index]
								a.files.push(temp);
								//a.files.push(new r(a.ruid, e))
							}, t.exec.call(a, 'FileDrop', 'clearFilesPath'))
		...
		function o() {
				...
				t.each(e, function(e) {
					...
					if (t)
						if (t.isFile) {
							var n = e.getAsFile();
							if (a(n)){
								_filesPath.push(t.fullPath.substring(1, t.fullPath.length));
								d.push(n)
							}
							//a(n) && d.push(n)
						} else i.push(t)
			...
			function c(e, t) {
				e.isFile ? e.file(function(e1) {
					if (a(e1)){
						_filesPath.push(e.fullPath.substring(1, e.fullPath.length));
						d.push(e1), t()
					}
					//a(e) && d.push(e), t()
				}, function() {
...
(function(e, t, n) {
	...
	o.addFileFilter("mime_types", function(e, t, n) {
		...
		function x(e, n) {
			...
			}), e.drop_element && o.each(e.drop_element, function(el) {
				s.push(function(n) {
					var s = new t.FileDrop(o.extend({}, u, {
						drop_zone: el
					}));
					s.onready = function() {
							var el = t.Runtime.getInfo(this.ruid);
							r.features.dragdrop = el.can("drag_and_drop"), i++, h.push(this), n()
						},
						s.ondrop = function(event, n) {
							if (event.target.files.length > e.maxDropFiles){
								e.dragleaveCallback && e.dragleaveCallback();
								e.filesOverflowCallback && e.filesOverflowCallback()
								return
							}
							r.addFile(this.files)
						},
						s.ondragenter = function(n) {
							e.dragenterCallback && e.dragenterCallback();
						},
			...
			addFile: function(e, n) {
				...
				function h(e) {
					...
					if (e instanceof t.File) {
						...
						h(new o.File(e))
					} else if (e instanceof t.Blob){
						(h(e.getSource()), e.destroy())
					} else if (e instanceof o.File) {
						(n && (e.name = n), u.push(function(t) {
							c(e, function(n) {
								n || (f.push(e), a.push(e), s.trigger("FileFiltered", e)), r(t, 1)
							})
						})) 
					} else if (t.inArray(i, ["file", "blob"]) !== -1) {
						var temp = new t.File(null, e);
						if (e.webkitRelativePath)
							temp.webkitRelativePath = e.webkitRelativePath;
						h(temp)
					} else if (i === "node" && t.typeOf(e.files) === "filelist") {
						t.each(e.files, h)
					} else if (i === "array") {
						(n = null, t.each(e, h))
					}
